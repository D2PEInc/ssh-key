...
从 GitHub 或 URL 返回多把公钥时，脚本会为每把公钥编号；输入 `1 2` 或 `1,2` 可以选择多把。序号仅用于选择，导入前仍须通过另一可信渠道核对对应的 `SHA256:` 指纹。

在已存公钥管理中，输入单个序号可删除一把公钥；输入 `1 2 5` 或 `1,2,5` 可原子删除多把公钥。重复序号会自动忽略。若选择会删除全部有效公钥，脚本仍会要求先确认密码备用连接。
...
- CLI 模式会强制比较远程公钥指纹；交互模式提供编号选择，但无法代替可信渠道核对。不要仅凭脚本刚显示的指纹判断公钥可信。
